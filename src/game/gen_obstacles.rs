...
pub fn gen_obstacles(x: i32, game_options: &GameOptions) -> Vec<(i32, i32)> {
  ...
  for _i in 0..(x*2) {
    if game_options.random_obstacles == false { break; };
  ...
  if let Ok(lines) = read_lines("assets/obstacles.txt") {
    ...
    for line in lines {
      if let Ok(cell) = line {
        ...
        if cell.is_empty() || regex.is_match(&*cell) { continue; };
        ...
        match parsed_cells {
          Err(_) => {
            println!("'{}' is not a valid obstacle. Define obstacle using start->end.", cell);
            continue;
          },
          Ok(v) => {
            ...
            if v[0] > x || v[1] > x {
              println!("Board doesn't contain value where '{}' starts or ends.", cell);
              continue;
            }
