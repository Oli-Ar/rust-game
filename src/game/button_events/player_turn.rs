use crate::game::Game;
use crate::structs::Obstacle::{Ladder, Snake};
use crate::structs::{ObstacleData, Player, RollData};
...
impl Game {
    pub fn player_turn(&mut self, player_num: i32) {
        ...
        let obstacles = if obstacle_data.len() > 0 {
            Some(obstacle_data)
        } else {
            None
        };
...
fn update_pos(player: &mut Player, roll_total: i32, game: &Game) -> i32 {
    ...
    player.score = if player.score + roll_total > 0 {
    ...
    player.cell = if player.score > 0
        && player.score <= game.game_options.board_size * game.game_options.board_size
    {
...
fn check_obstacle(
    ...
    if player.cell.as_ref().is_some() {
        ...
        if target_cell.start == true {
            ...
            update_pos(
                player,
                target_cell.end.unwrap() + 1 - target_cell.cell_number,
                &game,
            );
