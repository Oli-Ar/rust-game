use crate::game::Game;
use piston_window::{ellipse, Context, Ellipse, Event, G2d, PistonWindow, Size};
    ...
    window.draw_2d(e, |c, mut g, _device| {
        ...
        for player in game.players.as_ref().unwrap() {
            if let Some(cell) = &player.cell {
                ...
                let counter_size = (window_size.width + window_size.height)
                    / 2.0
                    / (game.game_options.board_size * 6) as f64;
                let cell_x =
                    cell.x as f64 * (window_size.width / game.game_options.board_size as f64);
                let cell_y = cell.y as f64
                    * ((window_size.height - 50.0) / game.game_options.board_size as f64)
                    + 50.0;
                ...
                match player.number {
                    ...
                    2 => {
                        draw_circle(
                            [0.0, 1.0, 1.0, 1.0],
                            counter_size,
                            cell_x + window_size.width / game.game_options.board_size as f64
                                - counter_size
                                - 5.0,
                            cell_y + 5.0,
                            c,
